/* =========================================================
   Dashboard rendering — extended dengan distribusi metrics
   ========================================================= */
(function () {
  ...
  D.render = function () {
    ...
    if (grid) {
      ...
      S.ITEMS.forEach(it => {
        ...
        const subInfo = (a.org > 0 || a.anorg > 0)
          ? `<div class="row"><span class="muted">Organik / Anorganik</span><span class="num"><span style="color:#16a34a">${U.fmt(a.org,{maximumFractionDigits:1})}</span> / <span style="color:#64748b">${U.fmt(a.anorg,{maximumFractionDigits:1})}</span> kg</span></div>` : '';
    ...
    trendChart = new Chart(cv.getContext('2d'), {
      ...
      options: {
        ...
        scales: {
          ...
          y: { beginAtZero: true, ticks: { color: chartTextColor(), callback: v => U.fmt(v, {maximumFractionDigits:0}) }, grid: { color: chartGridColor() } }
        }
      }
    });
  }
